...
```{r eval=FALSE, fig.align = 'default', warning = FALSE, fig.cap ="Fig. 3b", out.width="100%"}
train %>%
...
```{r eval=FALSE, fig.align = 'default', warning = FALSE, fig.cap ="Fig. 8b", out.width="100%"}
train %>%
...
Let's filter the data a little bit to remove the extreme (and the extremely suspicious) data points, and bin the data into a 2-d histogram (hidden plot). This plot shows that in log-log space the *trip\_duration* is increasing slower than linear for larger *dist*ance values.

```{r eval=FALSE, fig.align = 'default', warning = FALSE, fig.cap ="Fig. 10b", out.width="100%"}
train %>%
...
```{r eval=FALSE, fig.align = 'default', warning = FALSE, fig.cap ="Fig. 14a", out.width="100%"}
p1 <- train %>%
...
```{r eval=FALSE, fig.align = 'default', warning = FALSE, fig.cap ="Fig. 17b", out.width="100%"}
ny_map <- as.tibble(map_data("state", region = "new york:manhattan"))
...
weather_hourly <- as.tibble(fread("nyc_weather_2016.csv"))
```
...
```{r}
wc<-function(temp,vv){(35.74 + (0.6215 *temp)-(35.75 *vv^0.16)+(0.4275 *temp*(vv^0.16)))}

clean_weather_hourly <- weather_hourly %>% 
  mutate(Date = ymd(Date)) %>% 
  mutate(Hour = hour(strptime(weather_hourly$Time, "%I:%M %p"))) %>% 
  mutate(WindSpeed = as.numeric(ifelse(weather_hourly$`Wind Speed` == "Calm", "0", weather_hourly$`Wind Speed`))) %>% 
  mutate(Precip = as.numeric(ifelse(weather_hourly$Precip == "N/A", "0", weather_hourly$Precip))) %>% 
  mutate(Gust = as.numeric(ifelse(is.na(weather_hourly$`Gust Speed`), "0", weather_hourly$`Gust Speed`))) %>%
  mutate(Events = gsub(",","&",gsub("Ra", "Rain", gsub(("\r|\n|\t"),"", weather_hourly$Events)))) %>% 
  mutate(Events = ifelse(Events == "", "Clear", Events)) %>% 
  mutate(Humidity = as.numeric(gsub("%", "", weather_hourly$Humidity))/100) %>% 
  select(-`Gust Speed`, -`Wind Speed`, -V1, -Time) %>% 
  mutate(Windchill = round(wc(weather_hourly$Temp, weather_hourly$WindSpeed), 1))

```

```{r}
summary(clean_weather_hourly)
str(clean_weather_hourly)
```
...
```{r}
# left_join(train, clean_weather_hourly, by = c("date"="Date", "hour"="Hour"))
# Missing data!
```
...
foobar <- as.tibble(fread("fastest_routes_test.csv"))
...
```{r eval=FALSE, fig.align = 'default', warning = FALSE, fig.cap ="Fig. 24b", out.width="100%"}
p1 <- train %>%
  ggplot(aes(speed)) +
  geom_density(fill = "red", alpha = 0.5) +
...
At face value, the impact of the number of *turns* on the *total\_travel\_time* is very similar for *left\_turns* and *right\_turns*. A main reason behind this will be that the *total\_travel\_time* increases with increasing *number\_of\_steps* of any kind and that the number of *turns* is of course correlated with the *number\_of\_steps*. We will learn more from the percentage of turns per trip, which we will investigate in a moment. From these hidden plots we find the following results:

```{r eval=FALSE, fig.align = 'default', warning = FALSE, fig.cap ="Fig. 24c", out.width="100%"}
p1 <- train %>%
...
Another curious group of outliers can be found in the lower middle portion of the *trip\_duration* vs *total\_travel\_time* plot (hidden figure; where we encode the colour of the trips by their *vendor\_id*, to further highlight the impact of vendor 2 on the long-duration outliers on the top right and the few ones on the bottom right).

```{r eval=FALSE, fig.align = 'default', warning = FALSE, fig.cap ="Fig. 28a", out.width="100%"}
train %>%
...
Here, we are of course interested if and how strongly our correlate with the *trip\_duration*, the prediction of which is the ultimate goal of this challenge. But we also want to know whether our potential predictors are *correlated among each other*, so that we can reduce the collinearity in our data set and improve the robustness of our prediction. (The full correlation plot is included as a hidden figure.)
...
```{r}
which( colnames(combine)=="trip_duration" )

# combine_matrix <- sparse.model.matrix(trip_duration ~ ., data = combine)
```

```{r}
xg_train <- combine %>% filter(dset == "train") %>% select(-id, -trip_duration, -dset)

xg_test <- combine %>% filter(dset == "test") %>% select(-id, -trip_duration, -dset)

xg_train_target <- combine %>% filter(dset == "train") %>% select(trip_duration)

xg_test_target <- combine %>% filter(dset == "test") %>% select(trip_duration)


```
 
```{r}
combine_xgb <- xgboost(data = data.matrix(xg_train),
                       label = xg_train_target,
                       eta = 0.1,
                       max_depth = 5,
                       nrounds = 5,
                       subsample = 0.5,
...
```{r}
xgb_pred_y <- predict(combine_xgb, )
```
